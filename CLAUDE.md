...
- 🧪 **ระบบบำบัดน้ำเสีย — ต้นแบบแดชบอร์ด** (5 ก.ย. 69, ยังไม่เข้า DB): `web/wastewater.html` (Leaflet SPA หน้าเดียว อ่าน `web/data/wastewater.json` ที่สร้างจาก `scripts/build_wastewater_data.mjs [--fetch]`)
  แหล่งที่พบ: **คพ. DSPOT** `https://dspot.pcd.go.th/database/s?area=` หน้า Next.js ฝัง `__NEXT_DATA__.props.pageProps.reports` = 216 ระบบ (lat/lng, ความสามารถ, น้ำเข้าจริง, BOD/TSS/pH เข้า-ออก, ผู้เดินระบบ, ค่าบริการ, รูป) · JSON ตรง `/_next/data/<buildId>/database/s.json?area=` (buildId เปลี่ยนทุก deploy **ไม่มี CORS** → ต้องทำ Edge handler `pcd_dspot` parse HTML) · **กทม.** CKAN data.bangkok.go.th `wastewater.csv` 7 โรงควบคุมคุณภาพน้ำ พิกัด UTM 47N + `report-plant-qwater` รายเดือน · **สผ.** `onep.go.th/data/wastewater-treatment-system.csv` 106 ระบบเงินอุดหนุน ไม่มีพิกัด (geocode ตำบล 75/อำเภอ 17/ไม่ได้ 14) · อจน. (wma.or.th) ไม่มี API สาธารณะ · ชุดรายจังหวัดบน gdcatalog เป็นสถิติรวม
  ถ้าจะนำเข้าจริง: stations `pcd_wwtp` (station_type `wwtp`) + observations ปีละครั้ง + `site_report.wwtp_10km` · ต้องเพิ่ม `sources` 3 แถวใน seed
