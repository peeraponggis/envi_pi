...
- 🔄 **EIA (สผ. Smart EIA)**: ไม่มี API → `scripts/scrape_eia.mjs` (list = POST /site/eia 100 แถว/หน้า มี CSRF · detail = GET /eia/detail?id= เพื่อเอา "ที่ตั้งโครงการ" ตำบล/อำเภอ/จังหวัด) · list 13,510 โครงการเสร็จแล้ว · detail กำลังดึง (4 พร้อมกัน หน่วง 250 ms) → นำเข้าเป็น events source `onep_eia` พิกัดศูนย์กลางตำบล/อำเภอ · site_report `eia_20km`
- ⬜ ยังไม่ได้นำเข้า: การใช้ที่ดิน LDD รายจังหวัด · ชั้นคุณภาพลุ่มน้ำ (ไม่มีบน ArcGIS ของ DWR)
