...
- 🔎 **เซนเซอร์น้ำเรียลไทม์ที่พบ (5 ก.ย. 69 ยังไม่นำเข้า)**: (1) **กรมโรงงานฯ POMS** `poms.diw.go.th/factory-ws/get/factory-list?page=N` 800 โรงงานมี geom · `get/measurement-list/{id}` ค่าล่าสุดรายชั่วโมง WPMS น้ำทิ้ง (COD/BOD/Flow) 348 โรงงาน + CEMS อากาศ 490 · ไม่ต้องล็อกอิน แต่ **ไม่มี CORS** และประวัติ (`get/measurement/{id}`) ต้องล็อกอิน → ถ้าจะใช้ต้อง cron รายชั่วโมงเก็บเอง (2) **คพ. IWIS** `api-iwis.pcd.go.th/mst-station-with-summary?limit=200` 64 สถานีอัตโนมัติในแม่น้ำ (DO/pH/EC/Temp/Salinity ทุก 30 นาที, ย้อนหลัง 50 รอบในคำตอบ) มี CORS `*` แต่ก้อน 5.6 MB → ควรผ่าน Edge · รายละเอียดใน docs/แหล่งข้อมูลราชการ.md · ระบบบำบัดชุมชน 315 แห่งใน DB **ไม่มีเซนเซอร์** มีแค่ผลแล็บปีละครั้ง (DSPOT 159 ระบบ)
