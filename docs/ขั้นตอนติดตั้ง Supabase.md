...
**Edge v1.2.1 (3 ก.ย. 69 ค่ำ)**: แก้ dgr_wells ล้มทุกชั่วโมงเพราะ DGR ส่งบ่อ `no` ซ้ำในหน้าเดียว → `upsert()` dedupe ตามคีย์ onConflict ก่อนส่ง · deploy แล้ว cron วนครบ 117,634 บ่อ (ในฐาน 107,973) เช้า 4 ก.ย. 69 · วิธีตรวจ cron: `select jobname, schedule, active from cron.job` + `cron.job_run_details` (status/return_message) + แถวล่าสุดต่อ source ใน `ingest_runs` (ok/error/fn_version)
**ระบบบำบัดน้ำเสีย คพ. DSPOT (5 ก.ย. 69)**: รัน migration 1100 `wastewater.sql` (sources 3 แถว + site_report.wwtp_10km + cron `envi_pcd_dspot` `20 20 * * 1`) → deploy Edge v1.3.0 → `select public.trigger_ingest('pcd_dspot')` → ingest_runs ok 216 แถว · หน้า `web/wastewater.html` อ่าน stations source `pcd_dspot` ด้วย anon · สแนปช็อต กทม./สผ. สร้างใหม่ด้วย `node scripts/build_wastewater_data.mjs --fetch`
**เซนเซอร์น้ำเรียลไทม์ (5 ก.ย. 69 ค่ำ)**: migration 1200 `realtime_water.sql` (sources diw_poms/pcd_iwis + site_report poms_5km/iwis_20km + cron `envi_diw_poms` */15 · `envi_pcd_iwis` นาที 10 · purge เปลี่ยนเป็นทุกวัน เก็บ 7 วัน) → deploy Edge v1.4.0 → cron ยิงเอง (POMS 796 โรงงาน · IWIS 64 สถานี) · ถ้า Dashboard ค้าง ตรวจสถานะที่ `status.supabase.com/api/v2/status.json` แล้วตรวจผลผ่าน REST anon แทน: `stations?source_id=eq.diw_poms` + `v_station_latest`
