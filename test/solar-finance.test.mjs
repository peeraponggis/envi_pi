...
test('พลังงานเติมอากาศคิดจากเกณฑ์ออกแบบไทย ไม่ใช่ตัวเลขวรรณกรรมต่างประเทศ', async () => {
  ...
  near(r.bod_out, 80 * (1 - 0.9), 1e-9, 'บีโอดีน้ำออกคิดจากประสิทธิภาพกำจัดของประเภทระบบ');
  near(r.removed_kg, 10000 * (80 - 8) / 1000, 1e-9);
  near(r.o2_kg, r.removed_kg * 1.6, 1e-9, 'โหมดออกแบบใช้ค่าสูงสุดของช่วงเหมือนตัวอย่างของ คพ.');
  ...
  // เวลากักน้ำ 2 วัน (ค่าสูงสุดของช่วง เหมือนตัวอย่างของ คพ.) และกำลังกวน 2.5 กิโลวัตต์ต่อ 1,000 ลบ.ม.
  near(al.volume_m3, 10000 * 2, 1e-9);
  near(al.kwh_mix, 20000 / 1000 * 2.5 * 24, 1e-6);
  ...
  // ต้องบอกชัดว่าเป็นเฉพาะไฟเครื่องเติมอากาศ และตัวปรับในสนามมาจากตัวอย่างการคำนวณของ คพ. ไม่ใช่ค่าที่เราเดาเอง
  assert.ok(/เฉพาะไฟของเครื่องเติมอากาศ/.test(r.source) && /ตัวอย่างการคำนวณในเล่ม 3/.test(r.source));
...
test('ทวนตัวอย่างการคำนวณสระเติมอากาศในเล่ม 3 ของ คพ. ได้ตรงทุกบรรทัด', async () => {
  const { aerationEnergy } = await import('../web/js/sensor-catalog.js');
  // ตัวอย่างหัวข้อ 3.7 เล่ม 3: อัตราไหล 20,000 ลบ.ม./วัน แบ่ง 3 สระ → 6,700 ลบ.ม./วัน ต่อสระ
  const r = aerationEnergy('AL', { q_m3d: 6700 });
  near(r.bod_in, 80, 1e-9);                    // ตารางที่ 2.2 ท่อระบายรวม
  near(r.bod_out, 16, 1e-9);                   // 80 × (100 − 80) / 100
  near(r.removed_kg, 428.8, 1e-9);             // เอกสารระบุ 428,800 ก./วัน
  near(r.o2_kg / 24, 18, 0.2);                 // เอกสารปัดเศษ 428.8 ÷ 24 = 17.87 เป็น 18
  near(r.sae_field, 0.9, 1e-9);                // 1.8 × 0.5
  near(r.kwh_o2 / 24, 20, 0.2);                // 18 ÷ 0.9 = 20 กิโลวัตต์
  near(r.volume_m3, 13400, 1e-9);              // 6,700 × 2 วัน
  near(r.kwh_mix / 24, 33.5, 0.6);             // 13,400 ÷ 1,000 × 2.5 ≈ 34 กิโลวัตต์
  assert.equal(r.governing, 'mixing', 'เอกสารสรุปว่าการกวนผสมเป็นตัวควบคุมการออกแบบ');
  // ทั้งโรง 3 สระ ที่ 20,000 ลบ.ม./วัน
  near(r.kwh_day * 3 / 20000, 0.1206, 0.005);
